...
class NeuralNetworkModel(nn.Module):
    ...
    def train_model(self, input_data: list, target: list, epochs=100, batch_size=None):
        ...
        # Determine training buffer size threshold
        training_buffer_size = self.training_buffer_size if batch_size is None else batch_size * epochs

        # Check if buffer size is sufficient
        current_buffer_size = len(self.training_data_buffer)
        if current_buffer_size < training_buffer_size:
            ...
            log.info(f"Model {self.model_id}: Insufficient training data. "
                  ...
                  f"required: {training_buffer_size}")
